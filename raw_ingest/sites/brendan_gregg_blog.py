#!/usr/bin/env python3
"""
Brendan Gregg's blog (brendangregg.com/blog/...): Jekyll-style layout, div.site div.post body.

Run: cd raw_ingest && python sites/brendan_gregg_blog.py
"""
...
DEFAULT_URL = "https://www.brendangregg.com/blog/2026-02-07/why-i-joined-openai.html"
...
def _parse_post_date_line(s: str) -> str | None:
    s = s.strip()
...
def extract_brendan_gregg_blog_post(soup: BeautifulSoup, link_base: str) -> dict[str, Any]:
    meta: dict[str, Any] = {
    ...
    title_el = soup.select_one("div.site h2.big")
    if title_el:
        meta["title"] = _text(title_el)
    if not meta.get("title"):
        t = soup.find("title")
        meta["title"] = _text(t) if t else ""

    meta_el = soup.select_one("div.site p.meta")
    if meta_el:
        parsed = _parse_post_date_line(_text(meta_el))
        if parsed:
            meta["published_at"] = parsed
            meta["updated_at"] = parsed

    container = soup.select_one("div.site div.post")
    sections: list[dict[str, Any]] = []
    if not container:
        return {
            "meta": meta,
            "sections": sections,
            "parser_version": "raw_ingest.brendan_gregg_blog/0.1.0",
        }
...
def run_one(
    ...
    parser_output = extract_brendan_gregg_blog_post(soup, link_base=fetch_url)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest Brendan Gregg blog posts into data/docs")
    ap.add_argument("--fetch-url", default="", help="Blog post URL to fetch")
