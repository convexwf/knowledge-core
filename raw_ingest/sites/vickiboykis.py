#!/usr/bin/env python3
"""
Vicki Boykis blog (vickiboykis.com/...): Hugo Bear Blog; article body in <main><content>.

Run: cd raw_ingest && python sites/vickiboykis.py
"""
...
DEFAULT_URL = "https://vickiboykis.com/2026/03/04/antidote/"

DEFAULT_AUTHOR = "Vicki Boykis"
...
def extract_vickiboykis_post(soup: BeautifulSoup, link_base: str) -> dict[str, Any]:
    meta: dict[str, Any] = {
    ...
    content_el = None
    if main_el:
        content_el = main_el.find("content")

    h1 = main_el.find("h1") if main_el else soup.find("h1")
    if h1:
        meta["title"] = _text(h1)
    if not meta.get("title"):
        meta["title"] = (_meta_property(soup, "og:title") or _text(soup.find("title")) or "").strip()

    pub = _iso_from_published_string(_meta_property(soup, "article:published_time"))
    if not pub and main_el:
        t_el = main_el.find("time")
        if t_el and t_el.get("datetime"):
            pub = _iso_from_published_string(t_el["datetime"])
    if pub:
    ...
    if main_el:
        for a in main_el.select("p a[href*='/tags/']"):
            label = _text(a).lstrip("#").strip()
            if label:
                meta["tags"].append(label)
    ...
    if not content_el:
        return {
            "meta": meta,
            "sections": sections,
            "parser_version": "raw_ingest.vickiboykis/0.1.0",
        }
...
def run_one(
    ...
    parser_output = extract_vickiboykis_post(soup, link_base=fetch_url)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest vickiboykis.com posts into data/docs")
